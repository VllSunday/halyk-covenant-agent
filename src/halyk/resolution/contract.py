...
class UnresolvedReason(StrEnum):
    """Почему величины в ответе нет.

    Причины различаются не для отчётности: `not_found` означает, что документа с
    такой величиной в наборе нет, и это вопрос к переписи, а `conflicting_sources` —
    что документов два и они спорят, и это вопрос к разбору. Одно чинится чтением
    новых страниц, другое — правилом разрешения спора.
    """
...
class CandidateSource(BaseModel):
    """Место, которое модель сочла похожим, но недостаточным.

    Цитаты здесь нет и быть не может: если бы фрагмент с величиной нашёлся, это был
    бы факт, а не отказ. Адрес всё равно проверяется по переписи — выдуманное имя
    файла в отказе стоит ровно столько же, сколько в утверждении.
    """
...
class UnresolvedRequirement(BaseModel):
    """Требование, на которое модель отвечает отказом.

    Отказ — такой же ответ, как число, и потому обязателен. Молчание вместо него
    неотличимо от оборванного ответа и забытого элемента батча, а стоят они разного:
    честный отказ оставляет требование открытым, обрыв требует переспросить.
    """
    ...
    candidate_source_refs: tuple[CandidateSource, ...] = ()


class ResolverResponse(BaseModel):
    """Ответ модели на одного заёмщика.

    Ответ, в котором нет ни одной величины, законен: он означает отказ по каждому
    требованию, и это честнее правдоподобного числа. Но он обязан быть высказан —
    каждое требование батча называется ровно в одном исходе.
    """
...
На вход приходит список открытых требований и тексты страниц. Ответить нужно по
каждому требованию: молчание про требование — нарушение контракта, а не отказ.

Правила:

1. Каждое требование из списка названо ровно в одном месте ответа: в facts, в
   derivations или в unresolved_requirements. Одновременно в двух — нельзя.
2. Каждый элемент ответа ссылается на requirement_id из списка и повторяет его
   fact_kind. Требования, которого нет в списке, не выдумывай.
3. unit и currency обязаны совпадать с объявленными в требовании: величина заёмщика и
   величина группы, сумма в тенге и сумма в долларах — разные величины.
4. evidence — имя файла, номер страницы и дословный фрагмент, по которому видно
   число. Фрагмент будет проверен поиском по тексту страницы, поэтому не пересказывай
   его и не склеивай куски из разных мест.
5. Величину, которая напечатана строкой, возвращай в facts. Величину, которой строкой
...
6. Если требованию удовлетворяет больше одного числа, верни оба: выбирать будем не мы
   с тобой, а правило разрешения неоднозначности.
7. Величины в документах нет — отвечай unresolved_requirements с причиной:
   not_found — ничего похожего в документах нет;
   insufficient_evidence — место нашлось, но числа в нём не видно;
   conflicting_sources — числа два и они спорят;
   unsupported_derivation — величина выводится, но не из того, что раскрыто.
   Объясни отказ словами и назови страницы, на которые смотрел, если такие были.
   Честный отказ лучше правдоподобного числа.
8. adjustments — то, что документ предписывает изменить в конкретной операции реестра.
