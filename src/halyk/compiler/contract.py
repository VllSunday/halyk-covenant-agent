...
OUTPUT_CONTRACT = "covenant-formula-v3"
...
4. Денежные статьи заёмщика, которые можно собрать из реестра операций, всегда
   выражай через ledger_sum и категории из payload.categories. Это относится в том
   числе к выручке, операционным расходам, CAPEX, аренде, оплате труда, коммунальным
   расходам, налогам, процентам, страхованию и поступлениям финансирования. Слова
   «аудированная», «по МСФО» или «для целей ковенанта» не превращают такую статью во
   внешний факт: до исполнения код уже применяет к реестру предписанные документами
   корректировки.
5. Только величину, которой действительно нет в отдельных операциях реестра, объяви
   в required_facts вместе с периодом, единицей, валютой и областью. Например:
   консолидированный показатель Группы, порог существенности или раскрытая в
   примечании разовая статья. Показатель заёмщика и показатель группы — разные
   величины, даже когда называются одинаково.
6. quote — дословный фрагмент пункта, по которому видно порог и оператор. Он будет
   проверен поиском по тексту документа, поэтому не пересказывай его.
7. source_refs — страница, на которой этот фрагмент напечатан.
8. confidence снижай, когда формулировка допускает второе прочтение.
"""
