...
def dimension_of(unit: Unit) -> Dimension:
    """Размерность объявленной единицы.

    Процент и доля здесь неразличимы намеренно: они отличаются множителем, а не
    смыслом, и требовать от найденной величины ровно ту же единицу, что записана в
    требовании, значило бы отклонять правильный ответ из-за формы записи.
    """
    return _FROM_UNIT[unit]
